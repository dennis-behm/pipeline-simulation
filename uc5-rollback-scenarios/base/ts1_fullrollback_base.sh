...
# Pipeline Workspace
export PIPELINE_WORKSPACE=/u/dbehm/git/june-engagement-workspace/uc5-rollback-scenarios/ts1_fullrollback
...
if [ $rc -eq 0 ]; then
    wazideploy-deploy.sh -w $application/$branchName/${buildImplementation}build_$timestamp2 -a ${application} -x 'hlq=DBEHM.BASETEST' -t 'restore' -e EOLEB7-Integration.yml
    rc=$?
fi
