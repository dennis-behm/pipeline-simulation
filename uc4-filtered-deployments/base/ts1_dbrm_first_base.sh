...
echo "Deploy DBRM first"
if [ $rc -eq 0 ]; then
    wazideploy-deploy.sh -w $application/$branchName/${buildImplementation}build_$timestamp -a ${application} -x 'hlq=DBEHM.BASETEST' -t 'db2' -e EOLEB7-Integration.yml
    rc=$?
fi

if [ $rc -eq 0 ]; then
    wazideploy-evidence.sh -w $application/$branchName/${buildImplementation}build_$timestamp -o deploy/deployment-report_db2.html
    rc=$?
fi
echo "Deploy remaining artifacts"
if [ $rc -eq 0 ]; then
    wazideploy-deploy.sh -w $application/$branchName/${buildImplementation}build_$timestamp -a ${application} -x 'hlq=DBEHM.BASETEST' -o '--planSkipTags db2' -e EOLEB7-Integration.yml
    rc=$?
fi

if [ $rc -eq 0 ]; then
    wazideploy-evidence.sh -w $application/$branchName/${buildImplementation}build_$timestamp -o deploy/deployment-report_all.html
    rc=$?
fi
