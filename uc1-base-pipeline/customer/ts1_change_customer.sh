...
# Pipeline Workspace
export PIPELINE_WORKSPACE=/u/dbehm/git/june-engagement-workspace/ts1_change_customer
...
if [ $rc -eq 0 ]; then
    wazideploy-deploy.sh -w $application/$branchName/${buildImplementation}build_$timestamp -a ${application} -x 'hlq=DBEHM.CUSTTEST' -e EOLEB7-Integration.yml
    rc=$?
fi
